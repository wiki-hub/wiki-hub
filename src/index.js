...
export function save(opts) {
  ...
  return Promise
    ...
    .then(ensureWikiHubBranch(github, opts))
    .then(write(github, opts))
    .then(pullRequest(github, opts));
}

const body = 'Automatically generated by wikihub';
...
function ensureWikiHubBranch(github, opts) {
  return name => {
    const [owner, repo] = name.split('/');

    return github.listBranches({ owner, repo }).then(branches => {
      return branches.includes('wikihub') ||
        github.branch({ owner, repo, oldBranch: opts.branch, newBranch: 'wikihub' });
    }).then(returns(name));
  };
}
...
function write(github, opts) {
  return name => {
    const [owner, repo] = name.split('/');
    return github.write({ ...opts, owner, repo, branch: 'wikihub' }).then(returns(name));
  };
}
